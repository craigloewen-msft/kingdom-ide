...
[what kind of thing it is](#what-kind-of-thing-it-is).
...
|---|---|---|
| `type` | no | What kind of thing it is. `docker` if you leave it out, which is what every manifest written before kinds existed means. |
| `name` | yes | What you call it, and half the container's name. Letters, digits, `-` and `_` only, unique within the file. |
| `image` | yes | The image to run, **tag included**. `mongo:7`, not `mongo`. Docker's. |
| `port` | yes | The port the service listens on — and the port an agent reaches it at. |
| `volume` | no | A named Docker volume for the data. Without one, the data goes with the container. Docker's. |
...
A `type` Kingdom does not have is **refused by name**, in the manner of `env`
below:
...
Refused rather than quietly treated as a container, because a project that asked
for a runtime Kingdom cannot drive and silently got a different one would find
out from the container's behaviour an hour later, and read it as a bug in its
own code.
...
| A yellow row with a file path | That manifest does not parse — bad TOML, a duplicate name, a retired `env`, or a `type` naming a kind Kingdom does not have. The message says why and which file. **Nothing else in that file works either** until it is fixed. |
