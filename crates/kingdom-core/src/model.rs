...
impl WorkspaceMode {
    ...
    /// Whether preparing this mode needs git. Drives both the error the King
    /// sees for a non-git city and whether the repo lock is taken at all.
    pub fn needs_git(&self) -> bool {
        !matches!(self, WorkspaceMode::InPlace)
    }
}
...
pub enum NoteKind {
    /// A crown resource could not be claimed.
    Blocked,
...
mod transcript_tests {
    ...
    /// When every log line was an utterance, a lease refusal and a failed call
    /// were stored as things the *model* had said, and the next turn replayed
    /// them to it as its own prior words -- teaching it to answer in the voice of
    /// the plumbing. `said()` is the only door between a plan's log and a model,
    ...
    fn notes_never_reach_the_model_and_the_prompt_survives_them() {
        ...
        plan.note(NoteKind::Blocked, "Blocked: plan-9 holds Testburg files.");
