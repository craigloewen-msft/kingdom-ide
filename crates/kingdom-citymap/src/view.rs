...
pub fn CityMap(
    ...
    // difference.
    //
    // Selecting is suppressed in the rail, and that is not an oversight. In a
    // chamber `conversation.rs` force-sets `selected` from the open plan on
    // every render, so a click that set it would be overwritten a frame later
    // -- and a control that visibly does nothing is worse than no control. The
    // rail's map is a view; its head carries the way to the real one. Panning
    // and zooming stay live either way.
    let select = move |_| {
