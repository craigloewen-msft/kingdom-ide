...
/// How long one frame may spend building the world.
///
...
/// than when it stops working. Eight milliseconds leaves room in a 16 ms frame
/// for the renderer to draw the card and the bar under it, which is the whole
/// point of yielding at all.
...
impl RaisePlan {
    ...
    pub fn take(&mut self) -> Option<(Step, Range<usize>)> {
        while let Some(&(step, count)) = self.steps.get(self.at) {
            ...
            self.offset = end;
            self.done += step.weight() * (end - start) as f32;
...
pub fn raise_world(
    ...
    *winit = super::winit_for(true);
    ...
    while let Some((step, slice)) = job.plan.take() {
        spawn::spawn_step(
            &mut commands,
            &mut meshes,
            &mut materials,
    ...
    // The last slice. Everything from here is what `ViewerCommand::Load` used
    // to do inline once the whole world had been spawned.
...
mod tests {
    use super::*;
    use crate::map::{MapColor, MapPlaza, MapRect, MapSun, MapUnderside};
