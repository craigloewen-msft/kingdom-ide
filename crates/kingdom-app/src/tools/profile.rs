...
mod tests {
    ...
    /// The harness against a real page: samples come back, one per measured
    /// run, warmups discarded, and nothing reduced.
    #[tokio::test]
    async fn a_scenario_returns_one_raw_sample_per_measured_run() {
        ...
        let shop = Workshop::new(Workspace::in_place(dir.path().to_string_lossy()))
            .for_plan(kingdom_core::PlanId::new("scenario-live"));

        let page = "data:text/html,<button id=go>go</button><div id=out></div>\
                    <script>document.getElementById('go').addEventListener('click',()=>{\
                    document.getElementById('out').textContent='done'})</script>";
        ...
        let outcome = BrowserProfile
            .run(
                json!({
                    ...
                    "warmup": 1,
                    "throttle_rate": 1,
                    "reset": { "kind": "navigate", "url": page },
                    "steps": [
                        { "kind": "wait_selector", "selector": "#go" },
        ...
        let DeedOutcome::Done { output, .. } = outcome else {
            panic!("the scenario should run: {outcome:?}");
        };
        let report: Value = serde_json::from_str(&output).expect("a JSON report");
        let samples = report["raw_samples"].as_array().expect("raw samples");
        assert_eq!(samples.len(), 2, "one sample per measured run: {output}");
        assert_eq!(report["warmup_discarded"], 1);
        for key in ["script_ms", "long_tasks", "wall_ms", "dom_nodes"] {
            assert!(samples[0].get(key).is_some(), "sample is missing {key}: {output}");
        }
        for reduction in ["mean", "median", "stddev", "p95"] {
            assert!(
                report.get(reduction).is_none(),
                "the harness must not reduce the samples, found {reduction}"
            );
        }
    }
}
