...
mod tests {
    ...
    /// Where a Chromium lives on this machine, if one does.
    ///
    /// Read from the environment rather than hunted for: a test that searches
    /// the filesystem is a test that behaves differently on two machines, which
    /// is the opposite of what a test is for.
    fn chrome_available() -> bool {
        std::env::var("KINGDOM_CHROME_EXECUTABLE")
            .map(|p| std::path::Path::new(&p).is_file())
            .unwrap_or(false)
    }

    /// The whole reason this is a browser and not an HTTP client: state survives
    /// between calls, so a multi-step flow can be driven and then checked.
    ///
    /// Also pins that clicking goes through CDP rather than JS. The assertion
    /// is on a *React-style* listener attached with `addEventListener` — an
    /// element's `.click()` would satisfy a naive test while failing the real
    /// case the tool exists for, so the test is written to fail if the
    /// implementation ever regresses to that.
    ///
    /// Skipped rather than failed without Chrome: this is not something every
    /// machine can run, and a red suite on a laptop teaches nothing.
    #[tokio::test]
    async fn a_session_remembers_the_page_between_calls_and_clicks_land_as_real_events() {
        if !chrome_available() {
        ...
        let shop = Workshop::new(Workspace::in_place(dir.path().to_string_lossy()))
            .for_plan(kingdom_core::PlanId::new("browser-test"));

        let page = "data:text/html,<button id=b>go</button>\
                    <script>window.hits=0;\
                    document.getElementById('b')\
                    .addEventListener('click',()=>window.hits++)</script>";

        let navigated = BrowserNavigate
            .run(serde_json::json!({ "url": page }), &shop)
            .await;
        assert!(
        ...
        let clicked = BrowserClick
            .run(serde_json::json!({ "selector": "#b" }), &shop)
            .await;
        assert!(
            matches!(clicked, DeedOutcome::Done { .. }),
            "click should succeed: {clicked:?}"
        );

        // A separate call entirely. If the session did not persist this would
        // be a fresh about:blank and `window.hits` would not exist.
        ...
        assert!(
            output.contains('1'),
            "the click must fire a real listener, and the page must still be \
             the one navigated to: {output}"
        );
    }
    ...
    #[tokio::test]
    async fn without_chrome_the_court_is_told_rather_than_left_waiting() {
        if chrome_available() {
            return;
        }

        let shop = Workshop::new(Workspace::in_place("/tmp"))
            .for_plan(kingdom_core::PlanId::new("no-chrome"));

        let outcome = BrowserNavigate
            .run(serde_json::json!({ "url": "about:blank" }), &shop)
            .await;

        let DeedOutcome::Refused { reason } = outcome else {
            panic!("with no Chrome this must refuse, not appear to work");
        };
        assert!(
            reason.to_lowercase().contains("chrome"),
