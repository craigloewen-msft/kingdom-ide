...
crates/
  ...
  kingdom-citymap/  The map: every project drawn as a town on one disk, hanging
    ...
    engine/         Drawing it with Bevy (hydrate, plus native for its tests).
                    `stars.rs` is the one part not in the world: the projection
                    is orthographic, so a star out in the scene would have no
                    parallax and would *zoom* with the kingdom — it rides on the
                    camera in pixels instead
